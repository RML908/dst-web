...
import { DstMockApiService } from '@dst/lib/mock-api/mock-api.service';

@Injectable({
    providedIn: 'root'
})
export class DstMockApiInterceptor implements HttpInterceptor
{
    /**
     * Constructor
     */
    constructor(
        @Inject(Dst_MOCK_API_DEFAULT_DELAY) private _defaultDelay: number,
        private _dstMockApiService: DstMockApiService
    )
    {
    }
    ...
    intercept(request: HttpRequest<any>, next: HttpHandler): Observable<HttpEvent<any>>
    {
        ...
        const {
                  handler,
                  urlParams
              } = this._dstMockApiService.findHandler(request.method.toUpperCase(), request.url);
