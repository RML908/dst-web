...
import { ScrollbarGeometry, ScrollbarPosition } from '@dst/directives/scrollbar/scrollbar.types';

/**
 * Wrapper directive for the Perfect Scrollbar: https://github.com/mdbootstrap/perfect-scrollbar
 */
@Directive({
    selector: '[dstScrollbar]',
    exportAs: 'dstScrollbar'
})
export class DstScrollbarDirective implements OnChanges, OnInit, OnDestroy
{
    /* eslint-disable @typescript-eslint/naming-convention */
    static ngAcceptInputType_dstScrollbar: BooleanInput;
    /* eslint-enable @typescript-eslint/naming-convention */

    @Input() dstScrollbar: boolean = true;
    @Input() dstScrollbarOptions: PerfectScrollbar.Options;
    ...
    {
        // Enabled
        if ( 'dstScrollbar' in changes )
        {
            // Interpret empty string as 'true'
            this.dstScrollbar = coerceBooleanProperty(changes.dstScrollbar.currentValue);

            // If enabled, init the directive
            if ( this.dstScrollbar )
            {
                this._init();
            }
        ...
        // Scrollbar options
        if ( 'dstScrollbarOptions' in changes )
        {
            // Merge the options
            this._options = merge({}, this._options, changes.dstScrollbarOptions.currentValue);
