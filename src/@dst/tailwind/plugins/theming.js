...
const theming = plugin.withOptions((options) => ({
  ...
  }) =>
  {
     ...
     * CSS dst Properties using the colors from them. This allows switching
    ...
    addComponents(
      _.fromPairs(_.map(options.themes, (theme, themeName) => [
        ...
        ))), (value, key) => [[`--dst-${e(key)}`, value], [`--dst-${e(key)}-rgb`, chroma(value).rgb().join(',')]])))
      ]))
    );

    /**
     * Generate scheme based css dst properties and utility classes
     */
    const schemedstProps = _.map(['light', 'dark'], (colorScheme) =>
    {
      const isDark = colorScheme === 'dark';
      const background = theme(`dst.dstProps.background.${colorScheme}`);
      const foreground = theme(`dst.dstProps.foreground.${colorScheme}`);
      ...
      return {
        [(isDark ? darkSchemeSelectors : lightSchemeSelectors)]: {
          ...
          /**
           * If a dst property is not available, browsers will use
          ...
          /* Generate dst properties from dstProps */
          ..._.fromPairs(_.flatten(_.map(background, (value, key) => [[`--dst-${e(key)}`, value], [`--dst-${e(key)}-rgb`, chroma(value).rgb().join(',')]]))),
          ..._.fromPairs(_.flatten(_.map(foreground, (value, key) => [[`--dst-${e(key)}`, value], [`--dst-${e(key)}-rgb`, chroma(value).rgb().join(',')]])))
        }
      };
    });
      ...
      theme: {
        extend: {
          ...
          colors: _.fromPairs(_.flatten(_.map(_.keys(flattenColorPalette(normalizeTheme(options.themes.default))), (name) => [
            [name, `rgba(var(--dst-${name}-rgb), <alpha-value>)`],
            [`on-${name}`, `rgba(var(--dst-on-${name}-rgb), <alpha-value>)`]
          ])))
        },
        dst  : {
          dstProps: {
